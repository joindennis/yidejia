...
class api_en_tag {
    ...
    /**
     * api： en.tag.getGoodsByTagName
     * 功能：根据标签名称获取关联的商品ID列表
     * 调用：CT_Api::get();
     * 参数：$name: 标签名称
     */
    public function getGoodsByTagName() {
        $name = core::getInput("name");
        $data = $this->model->getGoodsListByTagName($name);
        if ($data) {
            //存在
            core::outPut(1, $data);
        } else {
