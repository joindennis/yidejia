 ...
 * 栏目：国际站API
 */
class api_en_recipient {

    private $model;

    public function __construct() {
        $this->model = new module_en_recipient();
    }

    /**
     * api： en.recipient.get
     * 功能：根据收件人ID获取收件人信息
     * 调用：CT_Api::get();
     * 参数：$id：收件人ID 必须
     */
    ...
    /**
     * api： en.recipient.getByBillingID
     * 功能：根据电子账单ID获取收件人信息
     * 调用：CT_Api::get();
     * 参数：$id：电子账单ID 必须
     */
    public function getByBillingID() {
        $id = core::getInput("id");
        $data = $this->model->getByBillingId($id);
        if ($data) {
            //存在
            core::outPut(1, $data);
        } else {
