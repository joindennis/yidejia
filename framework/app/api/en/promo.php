...
class api_en_promo {
    ...
    public function __construct() {
        $this->model = new module_en_promo();
    }
  ...
  /**
     ...
     * 参数：$id：皮肤分析id 必须
     */
    public function promoUsed() {
        $email = core::getInput("email");
        $data = $this->model->promoUsed($email);
        if ($data) {
            //存在
            core::outPut(1, $data);
        } else {
    ...
    /**
     * api： en.promo.clearCacheById
     * function：clear cache data by id
     * call：CT_Api::post();
     * param：$id: promo id  (must be assigned)
     */
