 ...
 * 功能: 皮肤分析器模型类
 */
class module_en_promo {

    private $db; //数据库对象
    private $cache; //缓存对象
    private $key_prefix = "EN_OAT_USER_PROMOCODES_"; //缓存key前缀
    private $table_name = 'oat_user_promocodes'; //表名称
    ...
    public function getByEmail($email) {
        ...
        $data = $this->cache->fetch($key);
        if (!$data) {
            $data = $this->db->fetch("select * from {$this->table_name} where email='$uid'");
            $this->cache->store($key, $data, array('life_time' => 600));
        }
        return $data;
    }
    ...
    public function updatePromo($data) {
    	if ($data['id']) {
	            ...
	            $sql = "update ".$this->table_name. " set promo='" . $data['promo'] . "' where user_id='" . $data['user_id'];
     ...
     * @return array
     */
    public function promoUsed($email) {
    	if ($email) {
		 
		 
	            $sql = "update ".$this->table_name. " set used=1 where email='".$email."'";
	        return $this->db->query($sql);
		}
		return false;
    }
