 ...
 * 功能: 皮肤分析器模型类
 */
class module_en_mmfMap {

    private $db; //数据库对象
    private $cache; //缓存对象
    private $key_prefix = "EN_OAT_MMF_MAP_"; //缓存key前缀
    private $table_name = 'oat_mmf_map'; //表名称
    ...
    public function getByUID($uid) {
        ...
        if (!$data) {
            $data = $this->db->fetch("select * from {$this->table_name} where id_salon=$uid");
    ...
    public function deleteBYID($id) {
        $key = $this->key_prefix . $id;
        $this->cache->delete($key);
        $sql = "delete from " . $this->table_name . " where id_salon=" . $id;
        return $this->db->query($sql);
    }
