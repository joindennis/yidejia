 ...
 * 功能: 皮肤分析器模型类
 */
class module_en_messages {

    private $db; //数据库对象
    private $cache; //缓存对象
    private $key_prefix = "EN_OAT_USER_MESSAGE_"; //缓存key前缀
    private $table_name = 'oat_user_message'; //表名称
    ...
    public function get($id) {
     //   $id = intval($id);
        ...
        if (!$data) {
            $data = $this->db->fetch("select * from {$this->table_name} where hash='$id'");
